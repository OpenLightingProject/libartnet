...
//		printf("port %i %i == %i , %i \n", i, p->data.admx.universe , n->ports.out[i].port_addr ,n->ports.out[i].enabled) ;
...
int handle_firmware(node n, artnet_packet p) {
	...
	if ( p->data.firmware.type == ARTNET_FIRMWARE_FIRMFIRST || p->data.firmware.type == ARTNET_FIRMWARE_UBEAFIRST) { 
		// a new transfer is initiated

		if( n->firmware.peer.s_addr == 0) { 
			//new transfer
			// these are 2 byte words, so we get a total of 1k of data per packet
			length = artnet_misc_nbytes_to_32( p->data.firmware.length ) * sizeof(p->data.firmware.data[0]) ;
			...
			block_length = min((unsigned int) length, ARTNET_FIRMWARE_SIZE * sizeof(p->data.firmware.data[0]) ) ;
			...
			if(block_length == length) {
				...
				if(n->callbacks.firmware_c.fh != NULL)
					n->callbacks.firmware_c.fh(n, n->firmware.ubea, n->firmware.data, n->firmware.bytes_total,  n->callbacks.firmware_c.data) ;
	...
	} else if ( p->data.firmware.type == ARTNET_FIRMWARE_FIRMCONT || p->data.firmware.type == ARTNET_FIRMWARE_UBEACONT ) {
		// continued transfer		
		length = artnet_misc_nbytes_to_32( p->data.firmware.length ) * sizeof(p->data.firmware.data[0])  ;
	...
	} else if ( p->data.firmware.type == ARTNET_FIRMWARE_FIRMLAST || p->data.firmware.type == ARTNET_FIRMWARE_UBEALAST ) {
		length = artnet_misc_nbytes_to_32( p->data.firmware.length ) * sizeof(p->data.firmware.data[0])  ;
		total_blocks = length / ARTNET_FIRMWARE_SIZE / 2 + 1 ;
		...
		if (n->firmware.peer.s_addr == p->from.s_addr
			...
			// do the callback here
			if(n->callbacks.firmware_c.fh != NULL)
				n->callbacks.firmware_c.fh(n, n->firmware.ubea, n->firmware.data, n->firmware.bytes_total / sizeof(p->data.firmware.data[0]) ,  n->callbacks.firmware_c.data) ;
